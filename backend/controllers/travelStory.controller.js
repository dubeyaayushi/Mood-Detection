...
export const addTravelStory = async (req, res) => {
    ...
    //convert visited date from milliseconds to Date Obejct 
...
export const deleteImage = async (req, res, next) => {
    ...
    if(!imageUrl){
        return next(errorHandler(400, "Image Parameter is required !"))
    }
...
export const editTravelStory = async(req, res, next) => {
    ...
    try{
        ...
        if(!travelStory){
            next(errorHandler(404,'travel storynot found!'))
        }

        const placeholderImageUrl = `http://localhost:3000/assets/photoplaceholder.png`

        travelStory.title = title
        travelStory.story = story
        travelStory.visitedLocation = visitedLocation
        travelStory.imageUrl = imageUrl || placeholderImageUrl
        travelStory.visitedLocation = parsedVisitedDate
