...
const Home = () => {
  ...
  try {
    const response = await axiosInstance.put(`/travel-story/${story._id}/analyze`);
    const updatedMood = response.data.mood;

    // Update state: replace mood in the correct story
    setAllStories(prevStories =>
      prevStories.map(s =>
        s._id === story._id ? { ...s, mood: updatedMood } : s
       
      )
    );
console.log(updatedMood)  } catch (error) {
                ...
                {allStories.map((item) => {
                  return (
                    <TravelStoryCard
                      ...
                      mood={item.mood}
                       onAnalyzeClick={() => handleAnalyze(item)}
